...
A sample iOS app showing **MVVM-C with [XCoordinator](https://github.com/quickbirdstudios/XCoordinator) 3** — built specifically to demonstrate the library's container coordinators side by side (including the new SwiftUI interop) and to give you a worked example of MVVM-C scene wiring with RxSwift.
...
A login → home → detail flow built end-to-end with XCoordinator 3 and MVVM-C. Use it as a reference for:

- **Four home containers driving one route** — the Home screen lets you pick between `HomeTabCoordinator` (`TabBarCoordinator`), `HomeSplitCoordinator` (`SplitCoordinator`), `HomePageCoordinator` (`PageCoordinator`), and `HomeSwiftUICoordinator` (a SwiftUI `ViewCoordinator`). All four drive the same `HomeRoute`, so you can compare them side-by-side. (`NavigationCoordinator` also drives the News/UserList/User/About flows.)
- **SwiftUI ⇄ UIKit interop** (XCoordinator 3) — `HomeSwiftUICoordinator` hosts a SwiftUI view in a `RoutingController` (UIKit → SwiftUI), resolves its router via `@Routing<HomeRoute>`, and embeds the UIKit `UserListCoordinator`/`NewsCoordinator` in a `TabView` via `WrappedRouter` (SwiftUI → UIKit). Tab selection routes through the coordinator and animates SwiftUI state via `Transition.withAnimation`.
- **MVVM-C scene wiring** via the `BindableType` protocol — each UIKit scene is a `ViewController` + `ViewModel` protocol triplet + `ViewModelImpl`, bound with [RxSwift](https://github.com/ReactiveX/RxSwift) and [Action](https://github.com/RxSwiftCommunity/Action).
...
## The home-coordinator picker

When you reach the Home screen, the app asks you to pick between `HomeTabCoordinator`, `HomeSplitCoordinator`, `HomePageCoordinator`, `HomeSwiftUICoordinator`, or a random one. This picker is the point of the example: all four coordinators expose the **same** `HomeRoute { case news; case userList }`, and `AppCoordinator` wraps whichever one you pick into `AppRoute.home(_:)` — which now carries an `any Router<HomeRoute>` (XCoordinator 3 replaced the `StrongRouter`/`UnownedRouter` wrappers with `any Router` existentials).

The takeaway: routes describe *what* should happen, coordinators decide *how*. Swapping the container coordinator changes the entire navigation chrome of your app — even from UIKit to SwiftUI — without touching a single view model or route definition. Open [`AppCoordinator.swift`](XCoordinator-Example/Coordinators/AppCoordinator.swift) to see the wrapping, then compare [`HomeTabCoordinator.swift`](XCoordinator-Example/Coordinators/HomeTabCoordinator.swift), [`HomeSplitCoordinator.swift`](XCoordinator-Example/Coordinators/HomeSplitCoordinator.swift), [`HomePageCoordinator.swift`](XCoordinator-Example/Coordinators/HomePageCoordinator.swift), and [`HomeSwiftUICoordinator.swift`](XCoordinator-Example/Coordinators/HomeSwiftUICoordinator.swift) to see four different `prepareTransition(for:)` implementations handling identical routes.
...
`SceneDelegate.scene(_:openURLContexts:)` (and the matching cold-launch path in `willConnectTo`) parses the URL via [`DeepLinkParser`](XCoordinator-Example/Common/DeepLinkParser.swift) into an `AppRoute`, then triggers it on the app's `any Router<AppRoute>`. For news, `AppRoute.newsDetail` resolves to
...
One gotcha worth knowing if you deep-link through a `PageCoordinator`: `deepLink` chains the next route inside each transition's completion handler, but `UIPageViewController.setViewControllers(…, animated: true)` silently skips its completion when the requested page is *already* on-screen. A deep link whose page step targets the already-visible page would stall there. Earlier versions of this example worked around it with a custom `Transition.setReliably`; **XCoordinator 3's stock `Transition.set` now fires the completion even for a no-op page set**, so [`HomePageCoordinator`](XCoordinator-Example/Coordinators/HomePageCoordinator.swift) just uses `.set` and the workaround was removed.
